...
@pytest.mark.asyncio
async def test_get_genre_list_cached(es_client, redis_client, make_get_request):
...
@pytest.mark.asyncio
async def test_get_genre_detailed_cashed(make_get_request, es_client, redis_client):
    genre_id = str(genre_list[0]['id'])
    name = genre_list[0]['name']
